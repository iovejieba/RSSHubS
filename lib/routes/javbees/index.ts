...
// 辅助函数：将文件大小字符串转换为字节数（供enclosure的length使用）
    ...
    try {
        ...
        const items = $('.card .columns')
            ...
            .map((item) => {
                ...
                // 8. 关键步骤：构建enclosure对象 (Folo解析所需)
                let enclosure = null;
                const lengthBytes = parseSizeToBytes(size); // 计算文件大小对应的字节数
                ...
                if (torrentLinkRaw) {
                    enclosure = {
                        ...
                        length: String(lengthBytes), // 转换为字符串
                    };
                }
                // 备选：使用磁力链接
                else if (magnetRaw) {
                ...
                // 9. 返回最终的Item对象（必须包含enclosure字段）
                return {
                    title: `${videoId} ${size}`,
                    pubDate: pubDate ? parseDate(pubDate, 'YYYY-MM-DD') : new Date(), // 确保有效日期
                    link: itemLink,
                    guid: `${videoId}-${lengthBytes}`.replace(/\s+/g, '-').replace(/[^a-zA-Z0-9\-]/g, ''), // 生成简洁Guid
                    description: art(path.join(__dirname, 'templates/description.art'), {
                        ...
                        description,
                        tags,
                        magnet: magnetRaw, // 传递原始链接，XML层会自动转义
                        torrent: torrentLinkRaw,
                        screenshots,
                    }),
                    author: tags.join(', '),
                    category: tags,
                    enclosure: enclosure, // 这是最关键的一行，确保enclosure对象被返回
                };
            });
